...
- Type: Optional
- Description: Step limit for sub-agent runs. Unset means no step limit — a long run is not itself a failure signal, and runaway sub-agents are already bounded by the timeout. Set it to additionally cap execution steps on resource-constrained deployments.
- Default: -
- Example: `200`
